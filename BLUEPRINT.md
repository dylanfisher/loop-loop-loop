...
- Deck FX includes a loop-delay control (`0.00s` to `60.00s`) that inserts a transport gap after each completed loop pass before retriggering playback. The setting persists in sessions and clip FX metadata, and supports simple automation for per-cycle gap changes.
